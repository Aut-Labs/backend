...
import { MultiSigner } from "../sdk/src";
...
const getHubDetailsPromise = async (sdk: AutSDK, hubAddress: string) => {
  return new Promise(async (resolve, reject) => {
    // try {
    //   const nova = sdk.initService<HubContract>(HubContract, hubAddress);
    ...
    //       await sdk.pluginRegistry.getPluginDefinitionByType(hubAddress, 1);
    //     if (pluginDefinition.data) {
    //       const nova = sdk.initService<Hub>(Hub, hubAddress);
    ...
    //       const isHubExpired = pastQuests === allQuests.length;
...
const getLeaderBoardHubDetailsPromise = async (
  sdk: AutSDK,
  hubAddress: string
) => {
  return new Promise(async (resolve, reject) => {
...
export class UserController {
  ...
  public getHubs = async (req, res) => {
    const promises = [];
    try {
      const sdk = await AutSDK.getInstance(false);
      const hubRes = await sdk.hubRegistry.contract.getHubs();

      const allDaos = [...hubRes.data];

      for (let index = 0; index < allDaos.length; index++) {
        const hubAddress = allDaos[index];
        promises.push(getHubDetailsPromise(sdk, hubAddress));
      }
  ...
  public getLeaderHubs = async (req, res) => {
    const promises = [];
    try {
      const sdk = await AutSDK.getInstance(false);
      ...
      const MAX_DAOS = -30;
      const allDaos = [...hubRes.data].splice(MAX_DAOS);

      for (let index = 0; index < allDaos.length; index++) {
        const hubAddress = allDaos[index];
        promises.push(getLeaderBoardHubDetailsPromise(sdk, hubAddress));
      }
