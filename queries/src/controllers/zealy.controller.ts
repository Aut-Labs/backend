...
import AutSDK, { fetchMetadata, Hub } from "../sdk/src";
...
export class ZealyController {
  ...
  public isAdmin = async (req, res) => {
    try {
      ...
      for (const hub of autID.joinedHubs) {
        const nova = sdk.initService<Hub>(Hub, hub.hubAddress);
  ...
  public has20Members = async (req, res) => {
    try {
      ...
      query GetAutIds {
        autIDs(where: { hubAddress: "${nova.address.toLowerCase()}" }, first: 10000){
          hubAddress
        }
      }
    `);
      ...
      if (numberOfMembers > 20) {
        return res
          .status(200)
          .send({ message: "Hub has more than 20 members" });
      }
  ...
  public has50Members = async (req, res) => {
    ...
    `);
      ...
      if (numberOfMembers > 50) {
        return res
          .status(200)
          .send({ message: "Hub has more than 50 members" });
      }
  ...
  public has100Members = async (req, res) => {
    ...
    `);
      ...
      if (numberOfMembers > 100) {
        return res
          .status(200)
          .send({ message: "Hub has more than 100 members" });
      }
