...
export class AutIDRouter {
  ...
  private init(): void {
    ...
    this._router.get(
      "/user/generateSigil/:hubAddress",
      this.userController.generateSigil
    );

    this._router.get("/user/nonce/:address", this.userController.getUserNonce);

    this._router.get("/user/novas", this.userController.getHubs);
    this._router.get("/user/leaderHubs", this.userController.getLeaderHubs);
